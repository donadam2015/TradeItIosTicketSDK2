    ...
    public func authenticateIfNeeded(
        ...
        let authenticationPromise = self.authenticationPromise ?? Promise { fulfill, reject in
            if error.requiresAuthentication() {
                self.authenticate(onSuccess: fulfill, onSecurityQuestion: onSecurityQuestion, onFailure: reject)
            } else {
                fulfill()
            }
        }

        authenticationPromise.then(execute: onSuccess).catch(execute: onFailure)
